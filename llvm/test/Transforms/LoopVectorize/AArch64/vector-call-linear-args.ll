...
; SVE_OR_NEON:    [[TMP13:%.*]] = call <vscale x 2 x i64> @sve_foo_linear_nomask(ptr [[TMP12:%.*]])
...
; SVE_TF:    [[TMP19:%.*]] = call <vscale x 2 x i64> @sve_foo_linear(ptr [[TMP18:%.*]], <vscale x 2 x i1> [[ACTIVE_LANE_MASK:%.*]])
...
; NEON:    [[TMP5:%.*]] = call <4 x i32> @neon_baz_vector_and_linear(<4 x i32> [[WIDE_LOAD:%.*]], ptr [[TMP4:%.*]])
...
; SVE_OR_NEON:    [[TMP15:%.*]] = call <vscale x 4 x i32> @sve_baz_vector_and_linear(<vscale x 4 x i32> [[WIDE_LOAD:%.*]], ptr [[TMP14:%.*]])
...
; SVE_OR_NEON-SAME: (ptr noalias [[A:%.*]], ptr readnone [[B:%.*]], i64 [[N:%.*]]) #[[ATTR0]] {
; SVE_OR_NEON:    [[TMP14:%.*]] = call <vscale x 2 x i64> @sve_foo_linear_nomask_2x(ptr [[TMP13:%.*]])
...
; NEON-SAME: (ptr noalias [[A:%.*]], ptr readnone [[B:%.*]], ptr readonly [[C:%.*]], i64 [[N:%.*]]) {
; NEON:    [[TMP5:%.*]] = call <4 x i32> @neon_quux_linear(ptr [[TMP3:%.*]], ptr [[TMP4:%.*]])
; NEON:    [[DATA:%.*]] = call i32 @quux(ptr [[GEPC:%.*]], ptr [[GEPB:%.*]]) #[[ATTR3:[0-9]+]]
;
; SVE_OR_NEON-LABEL: define void @test_linear_mixed_types
; SVE_OR_NEON-SAME: (ptr noalias [[A:%.*]], ptr readnone [[B:%.*]], ptr readonly [[C:%.*]], i64 [[N:%.*]]) #[[ATTR0]] {
; SVE_OR_NEON:    [[TMP15:%.*]] = call <vscale x 4 x i32> @sve_quux_linear_mask(ptr [[TMP13:%.*]], ptr [[TMP14:%.*]], <vscale x 4 x i1> shufflevector (<vscale x 4 x i1> insertelement (<vscale x 4 x i1> poison, i1 true, i64 0), <vscale x 4 x i1> poison, <vscale x 4 x i32> zeroinitializer))
; SVE_OR_NEON:    [[DATA:%.*]] = call i32 @quux(ptr [[GEPC:%.*]], ptr [[GEPB:%.*]]) #[[ATTR5:[0-9]+]]
;
; SVE_TF-LABEL: define void @test_linear_mixed_types
; SVE_TF-SAME: (ptr noalias [[A:%.*]], ptr readnone [[B:%.*]], ptr readonly [[C:%.*]], i64 [[N:%.*]]) #[[ATTR0]] {
; SVE_TF:    [[TMP21:%.*]] = call <vscale x 4 x i32> @sve_quux_linear_mask(ptr [[TMP19:%.*]], ptr [[TMP20:%.*]], <vscale x 4 x i1> [[ACTIVE_LANE_MASK:%.*]])
...
; SVE_OR_NEON:    [[TMP13:%.*]] = call <vscale x 4 x i32> @sve_bar_linear(i32 [[TMP12:%.*]])
...
; SVE_OR_NEON-SAME: (ptr noalias [[A:%.*]], i64 [[N:%.*]]) #[[ATTR0]] {
; SVE_OR_NEON:    [[TMP13:%.*]] = call <vscale x 4 x i32> @sve_bar_neg_linear(i32 [[TMP12:%.*]])
